...
onready var ghost_body = $Sprite/GhostBody
onready var ghost_eyes = $Sprite/GhostEyes
...
onready var start_climb = can_climb
onready var start_jump = can_double_jump
onready var start_glide = can_glide
onready var start_gravity = can_gravity
...
func _ready():
	if Global.current_level == 0:
		if Global.overworld_spawn != null:
			position = Global.overworld_spawn
	
	Global.connect("reset",self,"reset")
	Global.connect("abilities_changed",self,"check_abilities")
	Global.player = self
...
#		position = position.round()
	...
	if is_on_floor():
		...
		if round(velocity.x) != round(n_speed):
			velocity.x = (62*velocity.x + n_speed)/63
	else:
		if round(velocity.x) != round(n_speed):
	...
	if jump  || (buffer_jump && (on_ground || (jump_left > 0 && can_double_jump))):
		...
		if audio_j.get_playback_position() == 0 || audio_j.get_playback_position() >= 0.1 || !audio_j.playing:
			audio_j.play()
	elif state == STATES.Glide:
	...
	velocity = move_and_slide_with_snap(velocity.rotated(-grav_rot),gravity_normal*snap,-gravity_normal,true,4,deg2rad(85)).rotated(grav_rot)
	#velocity = move_and_slide_with_snap(velocity,Vector2(0,snap),Vector2.UP)

func get_input(): 
	movement_dir = Input.get_vector("left","right","null","null").x
	
	if can_gravity && (Input.is_action_just_pressed("left") || Input.is_action_just_pressed("right")):
		# -90 bis 90 normal
...
func calculate_gravity_normal():
	...
	if n_g != gravity_normal:
		var flip = sign(round(gravity_normal.y/n_g.y))
		gravity_normal = (0*gravity_normal + n_g)/1
		velocity.y *= flip
...
func calculate_sprite():
	...
	if movement_dir < 0:
		sprite.flip_h = true
		ghost_body.flip_h = true
		ghost_eyes.flip_h = true
		$HoldingPos.position.x = -12
	elif movement_dir > 0:
		sprite.flip_h = false
		ghost_body.flip_h = false
		ghost_eyes.flip_h = false
		$HoldingPos.position.x = 12
	
	if holding != null:
		holding.position = position + $HoldingPos.position
		var hp = $HoldingPos.position.x
...
func get_state():
	...
	if n_state == STATES.Fall:
		if is_on_wall() && state != STATES.Glide && can_climb && holding == null:
...
func check_abilities(_n = ""):
	can_climb = start_climb
	can_double_jump = start_jump
	can_glide = start_glide
	can_gravity = start_gravity
