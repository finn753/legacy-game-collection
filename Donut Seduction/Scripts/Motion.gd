...
func limit_speed(speed: Vector2,max_speed,delta):
	max_speed *= DELTA_MULTIPLY * delta
	if speed.x > max_speed:
		speed.x = max_speed
	elif speed.x < -max_speed:
		speed.x = -max_speed

	if speed.y > max_speed:
		speed.y = max_speed
	elif speed.y < -max_speed:
		speed.y = -max_speed
...
func get_target_motion(p,t,delta):
	var motion = Vector2(0,0)
	var buffer = Vector2(8,8)
	...
	if Tool.is_in_area_vector(p.position,t-buffer,t+buffer):
		return motion
	
	motion = Vector2(p.SPEED*DELTA_MULTIPLY,0)
